...
from annotation_generation.datatypes.parallelizability.TransformerFlagOptionList import TransformerFlagOptionListFilter, \
    TransformerFlagOptionListAdd, ChainTransformerFlagOptionList
from datatypes.BasicDatatypes import Flag, Option, ArgStringType
...
class ParallelizabilityInfoGeneratorSort(ParallelizabilityInfoGeneratorInterface):
    ...
    def generate_info(self) -> None:
        if self.does_flag_option_list_contains_at_least_one_of(["-c", "-C", "-u", "-z", "-R", "-s", "-m", "--files0-from", "--random-source"]):
            ...
            aggregator_spec = AggregatorSpec.make_aggregator_spec_custom_2_ary(spec_agg_cmd_name='sort',
