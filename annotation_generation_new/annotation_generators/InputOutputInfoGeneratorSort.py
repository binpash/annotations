...
class InputOutputInfoGeneratorSort(InputOutputInfoGeneratorInterface):
    ...
    def generate_info(self) -> None:
        ...
        if not self.does_flag_option_list_contains_at_least_one_of(["-o"]):
            self.set_implicit_use_of_stdout()
        if self.get_operand_list_length() == 0 and not self.does_flag_option_list_contains_at_least_one_of(["--files0-from"]):
