...
class CommandInvocationWithIOVars:
    ...
    def __init__(
        ...
        self.flag_option_list: List[Union[Flag, OptionWithIOVar]] = deepcopy(
            flag_option_list
        )
        self.operand_list: List[Union[ArgStringType, IOVar]] = deepcopy(operand_list)
        self.implicit_use_of_streaming_input: Optional[IOVar] = deepcopy(
            implicit_use_of_streaming_input
        )
        self.implicit_use_of_streaming_output: Optional[IOVar] = deepcopy(
    ...
    def is_aggregator_concatenate(
        self,
    ):  # needed since isinstance(_, Aggregator) does not work
        return False
    ...
    def substitute_inputs_and_outputs_in_cmd_invocation(
        self, inputs_from: List[IOVar], outputs_to: List[IOVar]
    ) -> None:
    ...
    def replace_var_consistently(self, from_var, to_var):
        assert (
            from_var in self.access_map
        )  # if this is not true, something went wrong before
        self.access_map[to_var] = self.access_map.pop(from_var)
        return to_var

    def replace_var(self, from_var, to_var):
        function_to_apply_to_var = (
            lambda el: self.replace_var_consistently(from_var, to_var)
            if el == from_var
            else el
        )
    ...
    def flat_map_non_names_aux_flag_option_list(self, function_to_apply):
        ...
        for i in range(len(self.flag_option_list)):
            ...
            if isinstance(flagoption, OptionWithIO):
                ...
                new_flag_option_list.append(
                    OptionWithIO(flagoption.option_name, new_option_arg)
                )
            else:
    ...
    def map_var(self, function_to_apply_to_vars):
        function_to_apply_to_anything = (
            lambda el: [function_to_apply_to_vars(el)] if isinstance(el, int) else [el]
        )
