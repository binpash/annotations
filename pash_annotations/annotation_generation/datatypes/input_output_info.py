...
from pash_annotations.datatypes.basic_datatypes_with_io import (
    ...
    OptionWithIO,
    get_from_original_stddescriptor_with_ioinfo,
    get_from_original_filename_with_ioinfo,
)
from pash_annotations.util import (
    compute_actual_el_for_input,
    compute_actual_el_for_output,
)
...
class InputOutputInfo:
    def __init__(
        ...
        implicit_use_of_streaming_input: Optional[
            FileNameOrStdDescriptorWithIOInfo
        ] = None,
        implicit_use_of_streaming_output: Optional[
            FileNameOrStdDescriptorWithIOInfo
        ] = None,
    ) -> None:
        self.flagoption_list_typer: List[
            Union[
                Tuple[Literal[WhichClassForArg.FILESTD], AccessKind],
                Tuple[Literal[WhichClassForArg.ARGSTRING], None],
                Tuple[Literal[WhichClassForArg.PLAINSTRING], None],
    ...
    def set_implicit_use_of_stdin(self, value: bool) -> None:
        if value:
            stdin_with_io = get_from_original_stddescriptor_with_ioinfo(
    ...
    def set_implicit_use_of_streaming_input(
    ...
    def set_implicit_use_of_stdout(self, value: bool) -> None:
        if value:
            stdout_with_io = get_from_original_stddescriptor_with_ioinfo(
    ...
    def set_implicit_use_of_streaming_output(
    ...
    def all_operands_are_streaming_outputs(self) -> None:
        ...
        self.operand_list_typer = [
            (WhichClassForArg.FILESTD, make_stream_output())
        ] * number_of_operands

    def all_operands_are_other_inputs(self) -> None:
        number_of_operands = len(self.operand_list_typer)
        self.operand_list_typer = [
            (WhichClassForArg.FILESTD, make_other_input())
        ] * number_of_operands

    def all_operands_are_other_outputs(self) -> None:
        number_of_operands = len(self.operand_list_typer)
        self.operand_list_typer = [
            (WhichClassForArg.FILESTD, make_other_output())
        ] * number_of_operands
    ...
    def set_all_operands_as_arg_string(self) -> None:
        ...
        self.operand_list_typer = [(WhichClassForArg.ARGSTRING, None)] * len(
            self.operand_list_typer
        )

    # methods to apply the InputOutputInfo to a command invocation

    def apply_input_output_info_to_command_invocation(
        ...
        flagoption_list_with_io: List[Union[Flag, OptionWithIO]] = [
            InputOutputInfo.apply_typer_to_flagoption(flagoption, typer)
            for (flagoption, typer) in zip(
                flagoption_list_original, self.flagoption_list_typer
            )
        ]
        # 2) transform operand list
        operand_list_original: List[Operand] = cmd_inv.operand_list
        operand_list_with_io_full: List[
            Union[ArgStringType, FileNameOrStdDescriptorWithIOInfo]
        ] = [
            InputOutputInfo.apply_typer_to_arg(operand.get_name(), typer)
            for (operand, typer) in zip(operand_list_original, self.operand_list_typer)
        ]
    ...
    def apply_typer_to_arg(
        ...
        if typer[0] == WhichClassForArg.FILESTD:
            ...
            if access.is_any_input():
                filename_or_stddescriptor: FileNameOrStdDescriptor = (
                    compute_actual_el_for_input(arg)
                )
            elif access.is_any_output():
                filename_or_stddescriptor: FileNameOrStdDescriptor = (
                    compute_actual_el_for_output(arg)
                )
            else:
                raise Exception("access which is neither any input nor output")
            if isinstance(filename_or_stddescriptor, FileName):
                return get_from_original_filename_with_ioinfo(
                    filename_or_stddescriptor, access
                )
            ...
            raise Exception(
                "no valid option for argument type WhichClassForArg: " + str(typer[0])
            )

    @staticmethod
    def apply_typer_to_flagoption(
        ...
        if isinstance(flagoption, Flag):
            ...
            assert (typer[0] == WhichClassForArg.FILESTD) or (
                typer[0] == WhichClassForArg.ARGSTRING
            )  # PLAINSTRING only for flags
            option_arg = flagoption.get_arg()
            option_arg_new: Union[
                ArgStringType, FileNameOrStdDescriptorWithIOInfo
            ] = InputOutputInfo.apply_typer_to_arg(option_arg, typer)
    ...
    def has_other_outputs(self):
        typer_lists = list(
            itertools.chain(self.flagoption_list_typer, self.operand_list_typer)
        )
