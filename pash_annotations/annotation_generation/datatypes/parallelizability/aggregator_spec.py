...
def make_aggregator_spec_adj_lines_merge() -> AggregatorSpec:
    return AggregatorSpecNonFunc(
        AggregatorKindEnum.ADJ_LINES_MERGE, is_implemented=False
    )
...
def make_aggregator_spec_adj_lines_func_from_cmd_inv_with_transformers(
    ...
    return AggregatorSpecFuncTransformer(
        kind=AggregatorKindEnum.ADJ_LINES_FUNC,
...
def make_aggregator_spec_adj_lines_func_from_string_representation(
    cmd_inv_as_str: str, is_implemented: bool = False
) -> AggregatorSpec:
    return AggregatorSpecFuncStringRepresentation(
        kind=AggregatorKindEnum.ADJ_LINES_FUNC,
...
def make_aggregator_spec_custom_2_ary_from_cmd_inv_with_transformers(
    ...
    return AggregatorSpecFuncTransformer(
        kind=AggregatorKindEnum.CUSTOM_2_ARY,
        flag_option_list_transformer=flag_option_list_transformer,
        # pos_config_list_transformer=pos_config_list_transformer,
        is_implemented=is_implemented,
    )
...
def make_aggregator_spec_custom_n_ary_from_cmd_inv_with_transformers(
    ...
    return AggregatorSpecFuncTransformer(
        kind=AggregatorKindEnum.CUSTOM_N_ARY,
        flag_option_list_transformer=flag_option_list_transformer,
        # pos_config_list_transformer=pos_config_list_transformer,
        is_implemented=is_implemented,
    )
...
def return_aggregator_conc_if_none_else_itself(
    arg: Optional[AggregatorSpec],
) -> AggregatorSpec:
...
class AggregatorSpecNonFunc(AggregatorSpec):
    def __init__(
        self,
        kind: Literal[
            AggregatorKindEnum.CONCATENATE,
            AggregatorKindEnum.ADJ_LINES_MERGE,
            AggregatorKindEnum.ADJ_LINES_SEQ,
        ],
        is_implemented: bool,
    ) -> None:
    ...
    def get_aggregator(
        ...
        if self.kind == AggregatorKindEnum.CONCATENATE:
            cmd_inv_cat = (
                CommandInvocationWithIOVars.make_cat_command_invocation_with_io_vars(
                    inputs_from, output_to
                )
            )
            return Aggregator.make_aggregator_from_cmd_inv_with_io(
                cmd_inv_cat, self.kind
            )
...
class AggregatorSpecFuncTransformer(AggregatorSpec):
    def __init__(
        self,
        kind: Literal[
            AggregatorKindEnum.ADJ_LINES_FUNC,
            AggregatorKindEnum.CUSTOM_2_ARY,
            AggregatorKindEnum.CUSTOM_N_ARY,
        ],
        flag_option_list_transformer: Optional[
            TransformerFlagOptionList
        ] = None,  # None translates to same as seq
        # pos_config_list_transformer: Optional[TransformerPosConfigList] = None,
        is_implemented: bool = False,
    ) -> None:
        AggregatorSpec.__init__(self, kind, is_implemented)
    ...
    def get_aggregator(
        ...
        aggregator_cmd_inv.flag_option_list = self.flag_option_list_transformer.get_flag_option_list_after_transformer_application(
        ...
        for input_id in inputs_from:
            assert not input_id in aggregator_cmd_inv.access_map and isinstance(
                input_id, IOVar
            )
            aggregator_cmd_inv.access_map[input_id] = make_stream_input()
        aggregator_cmd_inv.replace_var(
            aggregator_cmd_inv.implicit_use_of_streaming_output, output_to
        )
        return Aggregator.make_aggregator_from_cmd_inv_with_io(
            aggregator_cmd_inv, self.kind
        )
...
class AggregatorSpecFuncStringRepresentation(AggregatorSpec):
    ...
    def get_aggregator(
        ...
        agg_cmd_inv_with_io_vars = Aggregator(
            kind=self.kind,
