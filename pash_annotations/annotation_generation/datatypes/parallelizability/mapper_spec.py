...
class MapperSpec:
    def __init__(
        ...
        spec_mapper_cmd_name: Optional[
            str
        ] = None,  # None translates to original command name
        flag_option_list_transformer: Optional[
            TransformerFlagOptionList
        ] = None,  # None translates to same as seq transformer
    ...
    def get_mapper(
        ...
        if self.kind == MapperSpecKindEnum.SAME_AS_SEQ:
            mapper = Mapper.make_same_as_seq_mapper_from_command_invocation(
                original_cmd_invocation
            )
        elif self.kind == MapperSpecKindEnum.CUSTOM:
            cmd_name = return_default_if_none_else_itself(
                self.spec_mapper_cmd_name, original_cmd_invocation.cmd_name
            )
        ...
        if (
            len(aux_output_tos) == 0
        ):  # we previously checked that they are as many as specified in the parallelizer
            mapper.substitute_inputs_and_outputs_in_cmd_invocation(
            ...
            # trick for typing...
            new_operand_list = []
            for x in [input_from, output_to] + [
                aux_output_to for aux_output_to in aux_output_tos
            ]:
...
def make_mapper_spec_custom(
    spec_mapper_cmd_name: str,
...
def make_mapper_spec_custom_from_string_representation(
    ...
    return make_mapper_spec_custom(
        spec_mapper_cmd_name=cmd_inv.cmd_name,
