...
class ParallelizabilityInfo:
    def __init__(
        self,
        parallelizer_list: Optional[
            List[Parallelizer]
        ] = None,  # None translates to empty list
    ...
    def get_inferred_parallelizer_list(self):
        ...
        for parallelizer in self.parallelizer_list:
            if (
                self.is_commutative
                and parallelizer.splitter.is_splitter_consec_chunks()
            ):
                parallelizer_list.append(
                    make_parallelizer_round_robin_with_unwrap_from_other(parallelizer)
                )
        return parallelizer_list

    def unpack_info(self) -> Tuple[List[Parallelizer], bool, bool]:
        return (
            self.get_inferred_parallelizer_list(),
            self.round_robin_compatible_with_cat,
            self.is_commutative,
        )
