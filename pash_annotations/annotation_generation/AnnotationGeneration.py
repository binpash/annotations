...
inputoutput_info_generator_prefix_abs = (
    ANNOTATION_GENERATORS + "." + INPUTOUTPUT_INFO_FILENAME_MODULE_PREFIX
)
inputoutput_info_generator_file_module_names = [
    (
        inputoutput_info_generator_prefix_abs + name,
        INPUTOUTPUT_INFO_FILENAME_MODULE_PREFIX + name,
    )
...
parallelizability_info_generator_prefix_abs = (
...
parallelizability_info_generator_file_and_module_names = [
    (
        parallelizability_info_generator_prefix_abs + name,
        PARALLELIZABILITY_INFO_FILENAME_MODULE_PREFIX + name,
    )
    for name in DICT_CMD_NAME_TO_REPRESENTATION_IN_MODULE_NAMES.values()
]

for FILENAME_MODULE_PAIR in (
    inputoutput_info_generator_file_module_names
    + parallelizability_info_generator_file_and_module_names
):
...
def get_input_output_info_from_cmd_invocation(
    ...
    info_generator_class_for_cmd_repr = (
        DICT_CMD_NAME_TO_REPRESENTATION_IN_MODULE_NAMES.get(cmd_invocation.cmd_name)
    )
    try:
        info_generator_class_for_cmd = str_to_class(
            str(INPUTOUTPUT_INFO_FILENAME_MODULE_PREFIX)
            + str(info_generator_class_for_cmd_repr)
        )
...
def get_parallelizability_info_from_cmd_invocation(
    ...
    # Get the Generator
    info_generator_class_for_cmd_repr = str(
        PARALLELIZABILITY_INFO_FILENAME_MODULE_PREFIX
    ) + str(
        DICT_CMD_NAME_TO_REPRESENTATION_IN_MODULE_NAMES.get(cmd_invocation.cmd_name)
    )
    try:
