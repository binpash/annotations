...
class ParallelizabilityInfoGeneratorTr(ParallelizabilityInfoGeneratorInterface):
    ...
    def generate_info(self) -> None:
        ...
        # print("res: " + res)
        does_delete_newlines: bool = (
            self.does_flag_option_list_contain_at_least_one_of(["-d"])
            and self.does_last_set_effectively_contain_newline()
        )  # only allowed to have a single set
        # check for squeezing newlines
        does_squeeze_newlines: bool = (
            self.does_flag_option_list_contain_at_least_one_of(["-s"])
            and self.does_last_set_effectively_contain_newline()
        )
        if does_delete_newlines:
            ...
            self.append_to_parallelizer_list_cc_seq_adjf(
                "PLACEHOLDER: remove first line if empty", is_implemented=False
            )
            self.append_to_parallelizer_list_rr_seq_adjf(
                "PLACEHOLDER: remove first line if empty", is_implemented=False
            )
        else:
    ...
    def does_last_set_effectively_contain_newline(self) -> bool:
        ...
        if len(
            self.cmd_inv.operand_list
        ) == 1 and self.does_flag_option_list_contain_at_least_one_of(["-c"]):
            return not (
                last_operand_contains_newline or last_operand_contains_null_char
            )
