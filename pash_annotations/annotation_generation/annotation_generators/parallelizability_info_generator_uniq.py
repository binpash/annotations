    ...
    def generate_info(self) -> None:
        ...
        if not self.does_flag_option_list_contain_at_least_one_of(
            ...
            if self.does_flag_option_list_contain_at_least_one_of(["-c"]):
                ...
                aggregator_spec = (
                    make_aggregator_spec_adj_lines_func_from_string_representation(
                        cmd_inv_as_str="PLACEHOLDER:uniq_merge_count_uniq",
                        is_implemented=False,
                    )
                )
                parallelizer_cc_seq_adjf = make_parallelizer_consec_chunks(
                    aggregator_spec=aggregator_spec
                )
                parallelizer_rr_seq_adjf = make_parallelizer_round_robin(
                    aggregator_spec=aggregator_spec
                )
