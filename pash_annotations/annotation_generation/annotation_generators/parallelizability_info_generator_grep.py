...
from pash_annotations.annotation_generation.datatypes.parallelizability.parallelizer import (
    AdditionalInfoSplitterToMapper,
...
class ParallelizabilityInfoGeneratorGrep(ParallelizabilityInfoGeneratorInterface):
    ...
    def generate_info(self) -> None:
        if not self.does_flag_option_list_contain_at_least_one_of(["-q"]):
            ...
            if not self.does_flag_option_list_contain_at_least_one_of(
                ["-A", "-B", "-C", "-m"]
            ):
                if self.does_flag_option_list_contain_at_least_one_of(["-L", "-l"]):
                    ...
                    aggregator_spec = (
                        make_aggregator_spec_custom_2_ary_from_string_representation(
                            cmd_inv_as_str="PLACEHOLDER:sum_indiv_results_up",
                            is_implemented=False,
                        )
                    )
                    # cat input1 input2 | paste -s -d + - | bc
                elif self.does_flag_option_list_contain_at_least_one_of(
                    ["-n"]
                ) and self.does_flag_option_list_contain_at_least_one_of(["-b"]):
                    mapper_spec = make_mapper_spec_custom(
                        "PLACEHOLDER:grep_add_line_number_and_byte_offset",
                        is_implemented=False,
                    )
                    add_info_from_splitter = (
                        AdditionalInfoSplitterToMapper.LINE_NUM_AND_BYTE_OFFSET
                    )
                elif self.does_flag_option_list_contain_at_least_one_of(["-n"]):
                    mapper_spec = make_mapper_spec_custom(
                        "PLACEHOLDER:grep_add_line_number_offset", is_implemented=False
                    )
                    add_info_from_splitter = (
                        AdditionalInfoSplitterToMapper.LINE_NUM_OFFSET
                    )
                elif self.does_flag_option_list_contain_at_least_one_of(["-b"]):
                    mapper_spec = make_mapper_spec_custom(
                        "PLACEHOLDER:grep_add_byte_offset", is_implemented=False
                    )
            ...
            if (mapper_spec is None or mapper_spec.is_implemented) and (
                ...
                parallelizer_rr = make_parallelizer_round_robin(
                    mapper_spec=mapper_spec,
                    aggregator_spec=aggregator_spec,
                    info_splitter_mapper=add_info_from_splitter,
                )
