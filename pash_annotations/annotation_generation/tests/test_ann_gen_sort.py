...
from pash_annotations.datatypes.BasicDatatypes import Flag, Option, FlagOption, Operand, FileName, ArgStringType
from pash_annotations.datatypes.BasicDatatypesWithIO import make_stdout_with_access_output, make_stdin_with_access_stream_input
...
from pash_annotations.annotation_generation.datatypes.parallelizability.Splitter import Splitter, make_splitter_indiv_files, \
...
def test_sort_3() -> None:
    ...
    # Parallelizability Info
    para_info: Optional[ParallelizabilityInfo] = AnnotationGeneration.get_parallelizability_info_from_cmd_invocation(cmd_inv)
    assert para_info is not None and len(para_info.parallelizer_list) == 0 # because of stable sort
...
def test_sort_6() -> None:
    ...
    # Parallelizability Info
    para_info: Optional[ParallelizabilityInfo] = AnnotationGeneration.get_parallelizability_info_from_cmd_invocation(cmd_inv)
    assert para_info is not None and len(para_info.parallelizer_list) == 0 # because of files0-from
