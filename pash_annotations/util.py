...
def list_deduplication(list_to_dedup: List[TType]) -> List[TType]:
    ...
    [
        deduplicated_input_list.append(item)
        for item in list_to_dedup
        if item not in deduplicated_input_list
    ]
...
def return_empty_flag_option_list_if_none_else_itself(
    arg: Optional[List[FlagOption]],
) -> List[FlagOption]:
    if arg is None:
        return []
    else:
...
def return_empty_pos_config_list_if_none_else_itself(
    arg: Optional[List[OptionArgPosConfigType]],
